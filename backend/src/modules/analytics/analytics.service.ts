...
type Scope = {
  from: Date;
  to: Date;
  outlets: AccessibleOutlets | number;
...
export class AnalyticsService {
  constructor(
    ...
    private readonly snapshots: Repository<AnalyticsDailySnapshot>,
      ...
      const reports = await Promise.all(
        reportTypes.map(
          async (type) =>
            [
              type,
              await this.reports.getReport(type, reportQuery, outlets),
            ] as const,
        ),
      );
  ...
  async refreshDaily(user: User, query: AnalyticsQueryDto) {
    ...
    for (
      ...
      const payload = await this.dashboard(user, {
        ...query,
        from: businessDate,
        to: businessDate,
      });
      await this.snapshots.upsert(
        { outletId: query.outletId ?? 0, businessDate, version: 1, payload },
  ...
  async backfill(user: User, query: AnalyticsQueryDto) {
    const outlets = await this.access.getAccessibleOutletIds(user.id);
    const earliest = this.orders
      .createQueryBuilder('order')
      .select('MIN(order.created_at)', 'firstOrder');
    if (query.outletId !== undefined) {
      if (outlets !== ALL_OUTLETS && !outlets.includes(query.outletId))
        throw new ForbiddenException('You do not have access to this outlet');
      earliest.andWhere('order.outlet_id = :outletId', {
        outletId: query.outletId,
      });
    } else if (outlets !== ALL_OUTLETS) {
      earliest.andWhere('order.outlet_id IN (:...outletIds)', {
        outletIds: outlets,
      });
    }
    const firstRow = await earliest.getRawOne<{
      firstOrder: Date | string | null;
    }>();
    if (!firstRow?.firstOrder)
      return {
        range: null,
        refreshed: [],
    ...
    const timezone =
      typeof business.timezone === 'string' && business.timezone
        ? business.timezone
        : 'Asia/Kathmandu';
    const firstDate = new Intl.DateTimeFormat('en-CA', {
      timeZone: timezone,
    }).format(new Date(firstRow.firstOrder));
    const range = await this.snapshotRange({
      ...query,
      from: query.from ?? firstDate,
    });
    return this.refreshDaily(user, {
      ...query,
      from: range.from,
      to:
        query.to ??
  ...
  private async snapshotRange(query: AnalyticsQueryDto) {
    ...
    if (from > to)
      throw new ForbiddenException('from must be before or equal to to');
  ...
  private async scope(user: User, query: AnalyticsQueryDto): Promise<Scope> {
    ...
    if (
      ...
      outlets !== ALL_OUTLETS &&
      !outlets.includes(query.outletId)
    )
      throw new ForbiddenException('You do not have access to this outlet');
    if (outlets.length === 0)
    ...
    return {
      from,
      to,
      outlets: query.outletId === undefined ? outlets : query.outletId,
      query,
    };
  }
  ...
  private apply(
    ...
    qb.andWhere(`${alias}.created_at BETWEEN :from AND :to`, {
      from: scope.from,
      to: scope.to,
    });
    if (scope.outlets !== ALL_OUTLETS)
      qb.andWhere(`${alias}.${outletColumn} IN (:...outletIds)`, {
        outletIds: Array.isArray(scope.outlets)
          ? scope.outlets
          : [scope.outlets],
      });
    if (scope.query.orderSource)
      qb.andWhere(`${alias}.order_source = :orderSource`, {
        orderSource: scope.query.orderSource,
      });
    if (scope.query.orderType)
      qb.andWhere(`${alias}.order_type = :orderType`, {
        orderType: scope.query.orderType,
      });
    if (scope.query.departmentId)
      qb.andWhere(
        `EXISTS (SELECT 1 FROM order_items analytics_department_item WHERE analytics_department_item.order_id = ${alias}.id AND analytics_department_item.preparation_department_id = :departmentId)`,
        { departmentId: scope.query.departmentId },
      );
    return qb;
  }
  ...
  async overview(user: User, query: AnalyticsQueryDto) {
    ...
    const [sources, types, payments, itemRow] = await Promise.all([
      ...
      base
        .clone()
        .select('o.order_type', 'name')
        .addSelect('COUNT(*)', 'orders')
        .groupBy('o.order_type')
        .getRawMany(),
      (() => {
        const q = this.orders.manager
          .createQueryBuilder()
          .select('p.method', 'name')
          .addSelect('COALESCE(SUM(p.amount),0)', 'amount')
          .from('order_payments', 'p')
          .where("p.status='completed' AND p.type='payment'")
          .andWhere('p.paid_at BETWEEN :from AND :to', {
            from: scope.from,
            to: scope.to,
          });
        if (scope.outlets !== ALL_OUTLETS)
          q.andWhere('p.outlet_id IN (:...outletIds)', {
            outletIds: Array.isArray(scope.outlets)
              ? scope.outlets
              : [scope.outlets],
          });
        return q.groupBy('p.method').getRawMany();
      })(),
      base
        .clone()
        .innerJoin('order_items', 'items', 'items.order_id=o.id')
    ...
    return {
      ...
      kpis: {
        ...
        tax: n(row?.tax),
        serviceCharge: n(row?.serviceCharge),
      ...
      orderMix: {
        ...
        types: types.map((x) => ({ name: x.name, orders: n(x.orders) })),
      },
      paymentMix: payments.map((x) => ({ name: x.name, amount: n(x.amount) })),
    };
  }
  ...
  async products(user: User, query: AnalyticsQueryDto) {
    ...
    const qb = this.apply(
      this.orders.manager
        ...
        .innerJoin('foods', 'f', 'f.id=i.food_id')
    ...
    const categories = await qb
      .clone()
      .select('COALESCE(c.id,0)', 'categoryId')
      .addSelect("COALESCE(c.name,'Uncategorized')", 'category')
      .addSelect('SUM(i.quantity)', 'quantity')
      .addSelect('SUM(i.total_amount)', 'revenue')
      .addSelect('COUNT(DISTINCT o.id)', 'orders')
      .groupBy('c.id')
    ...
    return {
      foods: rows.map((r) => ({
        ...
        revenue: n(r.revenue),
        orders: n(r.orders),
        averagePrice: n(r.quantity) ? n(r.revenue) / n(r.quantity) : 0,
        share: total ? (n(r.revenue) / total) * 100 : 0,
      })),
  ...
  async inventory(user: User, query: AnalyticsQueryDto) {
    ...
    const stocks = db
      ...
      .addSelect('i.name', 'ingredient')
      .addSelect('MAX(i.reorder_level)', 'reorderLevel')
      .addSelect('MAX(i.minimum_stock)', 'minimumStock')
      .addSelect('SUM(s.quantity)', 'quantity')
      .addSelect('SUM(s.stock_value)', 'value')
      .addSelect('SUM(s.reserved_quantity)', 'reserved')
      .from('warehouse_ingredient_stocks', 's')
      .innerJoin('ingredients', 'i', 'i.id=s.ingredient_id')
      .innerJoin('warehouses', 'w', 'w.id=s.warehouse_id')
      .where('i.is_active=true');
    if (scope.outlets !== ALL_OUTLETS)
      stocks.andWhere('w.outlet_id IN (:...outletIds)', {
        outletIds: Array.isArray(scope.outlets)
          ? scope.outlets
          : [scope.outlets],
      });
    const stockRows = await stocks
      .groupBy('i.id')
      .addGroupBy('i.name')
    ...
    const tx = db
      ...
      .addSelect('COALESCE(SUM(t.quantity_out),0)', 'quantity')
      .from('ingredient_inventory_transactions', 't')
    ...
    if (scope.outlets !== ALL_OUTLETS)
      tx.andWhere('w.outlet_id IN (:...outletIds)', {
        outletIds: Array.isArray(scope.outlets)
          ? scope.outlets
          : [scope.outlets],
      });
    ...
    return {
      kpis: {
        inventoryValue: stockRows.reduce((s, r) => s + n(r.value), 0),
        lowStock: stockRows.filter(
          (r) =>
            n(r.quantity) > 0 &&
            n(r.quantity) <= Math.max(n(r.reorderLevel), n(r.minimumStock)),
        ).length,
        outOfStock: stockRows.filter((r) => n(r.quantity) <= 0).length,
        reservedStock: stockRows.reduce((s, r) => s + n(r.reserved), 0),
      },
      stock: stockRows
        .slice(0, 100)
        .map((r) => ({
          ingredientId: n(r.ingredientId),
          ingredient: r.ingredient,
          quantity: n(r.quantity),
          value: n(r.value),
        })),
      movement: movement.map((r) => ({
        type: r.type,
        quantity: n(r.quantity),
      })),
    };
  ...
  async customers(user: User, query: AnalyticsQueryDto) {
    const scope = await this.scope(user, query);
    const base = this.apply(this.orders.createQueryBuilder('o'), 'o', scope)
      .andWhere("o.status != 'cancelled'")
      .andWhere('o.customer_id IS NOT NULL');
    const priorOutlet =
      scope.outlets === ALL_OUTLETS
        ? ''
    ...
    const row = await base
      ...
      .addSelect(
        `COUNT(DISTINCT o.customer_id) FILTER (WHERE ${prior})`,
        'returningCustomers',
      )
      .setParameters({
        from: scope.from,
    ...
    return {
      ...
      trend: trendRows.map(
        (r: { date: string; newCount: string; returningCount: string }) => ({
          date: String(r.date).slice(0, 10),
          newCount: n(r.newCount),
          returningCount: n(r.returningCount),
        }),
      ),
    };
  }
