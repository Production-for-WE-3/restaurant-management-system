...
export class AttendanceController {
  ...
  @AllowWithoutPresence()
  @ApiOperation({
    summary:
      'Creates the permanent clock-in and clock-out QR codes for an outlet',
  })
  async setupQr(
    @Body() dto: SetupAttendanceQrDto,
  ...
  @ApiOperation({
    summary:
      'Clocks the logged-in employee in or out using an attendance QR code',
  })
  ...
  @AllowWithoutPresence()
  @ApiOperation({
    summary: 'Returns the logged-in staff member attendance status',
  })
  ...
  async clockOut(@Body() dto: ClockOutDto, @CurrentUser() user: User) {
    ...
    await this.outletAccess.assertOutletAccess(user.id, attendance.outletId);
  ...
  @RequirePermissions('attendance.manage')
  @ApiOperation({
    summary: 'Manually adjusts an attendance record (permission protected)',
  })
  async adjust(
  ...
  private assertTenant(
    ...
    if (
      suppliedTenantId !== undefined &&
      suppliedTenantId !== resolvedTenantId
    ) {
      throw new ForbiddenException(
        'Tenant ID does not match the authenticated tenant context',
      );
    }
