...
export class AssignmentsController {
  ...
  @Post('table-assignments/:id/unassign')
  ...
  async unassignTable(
    ...
    await this.outletAccess.assertOutletAccess(user.id, assignment.outletId);
  ...
  async listTableAssignments(
    ...
    return this.assignmentsService.listTableAssignments(
      parsedOutletId,
      accessible,
    );
  }
  ...
  @Post('order-assignments/:id/complete')
  @HttpCode(HttpStatus.OK)
  @RequirePermissions('employees.manage')
  @ApiOperation({ summary: 'Marks an order assignment complete' })
  async completeOrder(
    @Param('id', ParseIntPipe) id: number,
    @Body() dto: CompleteOrderAssignmentDto,
    @CurrentUser() user: User,
  ) {
  ...
  @Post('order-assignments/:id/served')
  @HttpCode(HttpStatus.OK)
  @RequirePermissions('employees.manage')
  ...
  async listOrderAssignments(
    ...
    return this.assignmentsService.listOrderAssignments(
      parsedOutletId,
      accessible,
    );
  }
  ...
  @ApiOperation({
    summary:
      'Staff dashboard aggregates (present, absent, on shift, assigned tables/orders)',
  })
  async getStaffDashboard(
    ...
    return this.assignmentsService.getStaffDashboard(
      parsedOutletId,
      accessible,
    );
  }
  ...
  async getPerformance(
    ...
    const employeeOutlets = await this.employeesService.getOutletIds(
      employee.id,
    );
    const accessibleOutlets = await this.outletAccess.getAccessibleOutletIds(
      user.id,
    );
    if (
      accessibleOutlets !== 'ALL' &&
      !employeeOutlets.some((id) => accessibleOutlets.includes(id))
    ) {
