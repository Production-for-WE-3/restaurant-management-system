...
export class UsersController {
  ...
  private scope(
    ...
    return request.tenantId ?? user.tenantId ?? undefined;
  }
  ...
  create(
    ...
    if (tenantId === undefined)
      throw new ForbiddenException('User creation requires a tenant hostname');
  ...
  @RequirePermissions('users.manage')
  @ApiOperation({
    summary: "Resets a user's password without revealing the old password",
  })
  resetPassword(
    @Param('id', ParseIntPipe) id: number,
  ...
  @RequirePermissions('users.manage')
  @ApiOperation({
    summary: "Disables the user's linked employee account",
  })
  deactivate(
    @Param('id', ParseIntPipe) id: number,
    @CurrentUser() user: User,
