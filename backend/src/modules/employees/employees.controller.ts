...
export class EmployeesController {
  ...
  findAllPositions(@CurrentUser() user: User) {
    return this.employeesService.findAllPositions(user.tenantId ?? undefined);
  }

  @Post('positions')
  @RequirePermissions('employees.manage')
  @ApiOperation({ summary: 'Creates a position' })
  createPosition(@Body() dto: CreatePositionDto, @CurrentUser() user: User) {
    return this.employeesService.createPosition(
      dto,
  ...
  removePosition(@Param('id', ParseIntPipe) id: number) {
    return this.employeesService.removePosition(id);
  }
  ...
  assignPositionPermission(
    @Param('id', ParseIntPipe) id: number,
    @Body() dto: AssignPositionPermissionDto,
    @CurrentUser() user: User,
  ) {
    return this.employeesService.assignPositionPermission(
      id,
      dto.permissionId,
      user.id,
    );
  ...
  @Get('employees/:id/outlets')
  @RequirePermissions('employees.view')
  listOutlets(@Param('id', ParseIntPipe) id: number) {
    return this.employeesService.listOutlets(id);
  }

  @Post('employees/:id/outlets')
  @RequirePermissions('employees.manage')
  async assignOutlet(
    @Param('id', ParseIntPipe) id: number,
    ...
    const employee = await this.employeesService.findOne(id);
    await this.outletAccess.assertOutletAccess(user.id, dto.outletId);
    return this.employeesService.assignOutlet(
      employee.id,
      dto.outletId,
      user.id,
    );
  ...
  @Delete('employees/:id/outlets/:outletId')
  @HttpCode(HttpStatus.NO_CONTENT)
  @RequirePermissions('employees.manage')
  async removeOutlet(
    @Param('id', ParseIntPipe) id: number,
  ...
  @Delete('employees/:id/departments/:departmentId')
  @HttpCode(HttpStatus.NO_CONTENT)
  @RequirePermissions('employees.manage')
  ...
  private async assertEmployeeAccess(
    ...
    const employeeOutlets =
      await this.employeesService.getOutletIds(employeeId);
    if (!employeeOutlets.some((outletId) => accessible.includes(outletId))) {
