...
function localParts(
  date: Date,
  timezone: string,
): { year: number; month: number; day: number; minute: number } {
...
export class OperatingHoursService {
  constructor(
    @InjectRepository(OutletOperatingHours)
    private readonly repo: Repository<OutletOperatingHours>,
  ...
  async update(
    ...
    const config =
      ...
      this.repo.create({
        ...
        closingTime: null,
        timezone: null,
        lastClosingBoundaryAt: null,
      });
    Object.assign(config, dto);
    if (
      config.openingTime &&
      config.closingTime &&
      config.openingTime.slice(0, 5) === config.closingTime.slice(0, 5)
    )
      throw new BadRequestException(
        'openingTime and closingTime must be different',
      );
    ...
    if (config.enabled) {
      if (!config.openingTime || !config.closingTime)
        throw new BadRequestException(
          'Enabled operating hours require openingTime and closingTime',
        );
  ...
  async getStatus(outletId: number): Promise<OperatingStatus> {
    ...
    const timezone =
      config?.timezone ?? String(business.timezone ?? 'Asia/Kathmandu');
    if (!config?.enabled || !config.openingTime || !config.closingTime)
      return {
        enabled: false,
        isOpen: true,
        openingTime: config?.openingTime ?? null,
        closingTime: config?.closingTime ?? null,
        timezone,
        nextOpeningAt: null,
        nextClosingAt: null,
      };
    ...
    const open = minutes(config.openingTime);
    const close = minutes(config.closingTime);
    const overnight = close < open;
    const todayOpen = dateAtLocal(now, timezone, open, 0);
    const todayClose = dateAtLocal(now, timezone, close, overnight ? 1 : 0);
    const previousClose = dateAtLocal(now, timezone, close, overnight ? 0 : -1);
    const isOpen = overnight
      ? nowLocal.minute >= open || nowLocal.minute < close
      : nowLocal.minute >= open && nowLocal.minute < close;
    const nextOpening = isOpen
      ? null
      : nowLocal.minute < open
        ? todayOpen
        : dateAtLocal(now, timezone, open, 1);
    const nextClosing = isOpen
      ? overnight && nowLocal.minute < close
        ? dateAtLocal(now, timezone, close, 0)
        : todayClose
      : null;
    void previousClose;
    return {
      enabled: true,
      isOpen,
      openingTime: config.openingTime.slice(0, 5),
      closingTime: config.closingTime.slice(0, 5),
      timezone,
      nextOpeningAt: nextOpening?.toISOString() ?? null,
      nextClosingAt: nextClosing?.toISOString() ?? null,
    };
  }

  async isOpen(outletId: number): Promise<boolean> {
    return (await this.getStatus(outletId)).isOpen;
  }

  async getEnabledConfigs(): Promise<OutletOperatingHours[]> {
    return this.repo.find({ where: { enabled: true } });
  }

  getClosingBoundary(
    config: OutletOperatingHours,
    now = new Date(),
  ): Date | null {
    if (!config.enabled || !config.openingTime || !config.closingTime)
      return null;
  ...
  async markClosingBoundary(
    configId: number,
    boundary: Date,
  ): Promise<boolean> {
    const result = await this.repo
      .createQueryBuilder()
      .update(OutletOperatingHours)
      .set({ lastClosingBoundaryAt: boundary })
      .where(
        'id = :id AND (last_closing_boundary_at IS NULL OR last_closing_boundary_at < :boundary)',
        { id: configId, boundary },
      )
      .execute();
  ...
  async assertOperational(outletId: number): Promise<void> {
    ...
    if (
      ...
      context.userId !== null &&
      (await this.outletAccess.canAccessOutlet(context.userId, outletId))
    ) {
