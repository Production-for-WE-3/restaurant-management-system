...
const BASE_PATH = "/operational/staff/tables"

export default function StaffTablesPage() {
  const router = useRouter()
  const params = useSearchParams()
  const deepLinkOrderId = params.get("orderId")
  ...
  function handleStartSaleOpenChange(open: boolean) {
    startSale.onOpenChange(open)
    if (!open && preselectedTableId !== undefined) {
  ...
  return <div className="flex flex-1 flex-col gap-3">
    ...
    {startSale.mounted && effectiveOutletId && <StartSaleDialog open={startSale.open} onOpenChange={handleStartSaleOpenChange} outletId={effectiveOutletId} onSaleStarted={(orderId) => router.push(`${BASE_PATH}?orderId=${orderId}`)} preselectedTableId={preselectedTableId} tables={tables.data?.data ?? []} />}
  </div>
}
