                          ...
                          {positions
                            ...
                            .map((position) => (
                              <div key={position.id} className="flex items-center gap-2">
                                <Checkbox
                                  id={`cash-position-${position.slug}`}
                                  checked={selected.includes(position.slug)}
                                  disabled={!canManage}
                                  onCheckedChange={(checked) => {
                                    field.onChange(
                                      checked === true
                                        ? [...selected, position.slug]
                                        : selected.filter((slug) => slug !== position.slug),
                                    )
                                  }}
                                />
                                <Label htmlFor={`cash-position-${position.slug}`}>{position.name}</Label>
                              </div>
                            ))}
                        </div>
                        <FormMessage />
