...
export default function PosPage() {
  ...
  } = useOrderDeepLink({ basePath: "/operational/pos", outletId, deepLinkOrderId, deepLinkTableId })
  ...
  useEffect(() => {
    if (isCompletedOrder) router.replace("/operational/pos")
    ...
    <div className="flex h-[calc(100vh-8rem)] flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {activeOrderId ? (
            <Button variant="outline" onClick={() => router.push("/operational/pos")}>
              New sale
            </Button>
          ) : (
            effectiveOutletId &&
      ...
      {!effectiveOutletId ? (
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-3 text-center">
          ...
          <Button onClick={() => router.push("/operational/pos")}>Back to floor</Button>
        </div>
      ) : isCompletedOrder ? (
      ...
      {needsOrderChooser && deepLinkSession && effectiveOutletId && (
        <TableOrdersDialog
          ...
          onSelectOrder={(orderId) => router.replace(`/operational/pos?orderId=${orderId}`)}
        />
      )}
      ...
      {startSale.mounted && effectiveOutletId && (
        <StartSaleDialog
          ...
          outletId={effectiveOutletId}
          onSaleStarted={(orderId) => router.push(`/operational/pos?orderId=${orderId}`)}
